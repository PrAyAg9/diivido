...
const ELEVEN_LABS_VOICE_ID = process.env.ELEVEN_LABS_VOICE_ID || 'EXAVITQu4vr4xnSDxMaL'; // Default voice
class AIController {
    constructor() {
        ...
        this.processVoiceCommand = async (req, res) => {
            ...
            try {
                ...
                if (!userId) {
                    const fallbackText = "Hi there! I'm your expense-splitting assistant. I'd love to help you manage your finances!";
                    res.json({
                        text: fallbackText,
                ...
                const isNudgeRequest = nudgeKeywords.some((keyword) => transcript.toLowerCase().includes(keyword));
                const isBalanceRequest = balanceKeywords.some((keyword) => transcript.toLowerCase().includes(keyword));
                const isQuickDrawRequest = quickDrawKeywords.some((keyword) => transcript.toLowerCase().includes(keyword));
                if (isNudgeRequest) {
          ...
          Format your response as if you're speaking directly to ${user.fullName}, 
        ...
        `;
                    ...
                    if (friendName) {
                        console.log(`🎯 Nudge sent to ${friendName}: Witty money reminder from ${user.fullName}!`);
                    }
                    res.json({
                        text: aiResponse,
                        audioUrl,
                    ...
                    // Return balance info
                    const balanceText = userData.balance.netBalance > 0
                        ...
                        : `Hey ${user.fullName}! You owe $${userData.balance.totalOwed} to friends, and they owe you $${userData.balance.totalOwedToUser}. Net: you owe $${Math.abs(userData.balance.netBalance)}. Time for some friendly nudges! 😉`;
...
Want to start a Quick Draw challenge now? Just add an expense and choose the Quick Draw option!`;
          ...
          Their balance: ${userData.balance.netBalance > 0
        ...
        this.generateReminderMessage = async (req, res) => {
            var _a, _b;
            try {
                ...
                const userData = await this.getUserContextData(((_b = req.user) === null || _b === void 0 ? void 0 : _b.id) || '');
                const prompt = `
        ...
        Context: ${senderUser.fullName} has ${userData.friends.length} friends and is in ${userData.groups.length} groups.
      ...
      `;
        ...
        this.getVoiceBalanceSummary = async (req, res) => {
            var _a;
            try {
                const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
                if (!userId) {
                    const fallbackText = "Hi! I'd love to show you your balance, but I need you to be logged in first!";
                ...
                // Check if Gemini API key is configured
                if (!process.env.GEMINI_API_KEY) {
                    const fallbackText = `Hey ${user.fullName}! You have ${userData.friends.length} friends and ${userData.groups.length} groups. Your current balance: ${userData.balance.netBalance >= 0
        ...
        - Net balance: ${userData.balance.netBalance >= 0
      ...
      `;
        ...
        this.sendWittyNudge = async (req, res) => {
            var _a;
            try {
                const { friendName, message, customMessage } = req.body;
                const userId = ((_a = req.user) === null || _a === void 0 ? void 0 : _a.id) || 'demo-user-123';
                const user = await user_model_1.User.findById(userId);
                if (!user) {
                    res.status(404).json({ error: 'User not found' });
                    return;
                ...
                // Find the friend
                const userData = await this.getUserContextData(userId);
                const friend = userData.friends.find((f) => f.fullName.toLowerCase().includes(friendName.toLowerCase()));
        ...
        this.getExpenseSuggestions = async (req, res) => {
            try {
      ...
      `;
        ...
        this.getUserContextData = async (userId) => {
            try {
                // Get user's friends
                ...
                const friends = friendRequests.map((req) => {
                    const friend = req.fromUserId._id.toString() === userId
                        ? req.toUserId
                        : req.fromUserId;
                    return {
                        id: friend._id,
                ...
                const recentExpenses = await expense_model_1.Expense.find({
                ...
                // Calculate balance
                const expensesOwedToUser = await expense_model_1.Expense.find({ paidBy: userId });
                const expensesUserOwes = await expense_model_1.Expense.find({ 'splits.userId': userId });
                let totalOwedToUser = 0;
                let totalOwed = 0;
                // Calculate what others owe to user
                for (const expense of expensesOwedToUser) {
                    const unpaidSplits = expense.splits.filter((split) => !split.paid && split.userId.toString() !== userId);
                    totalOwedToUser += unpaidSplits.reduce((sum, split) => sum + split.amount, 0);
                }
                ...
                return {
                    ...
                    recentExpenses: recentExpenses.map((expense) => {
                        ...
                        return ({
                            ...
                            groupName: ((_a = expense.groupId) === null || _a === void 0 ? void 0 : _a.name) || 'Personal',
                            date: expense.date,
                        });
                    }),
                    balance: {
        ...
        this.generateSpeech = async (text, voiceId = ELEVEN_LABS_VOICE_ID) => {
            try {
